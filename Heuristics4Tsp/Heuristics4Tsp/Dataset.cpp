...
Dataset Dataset::randomNDataset()
{
	...
	dataset.output_stats = baseOutputFolder.append("RANDheuristicsStats.txt");
	...
	for (int s = 0; s < sizes.size(); s++)
	{
		...
		for (int i = 0 ; i < 1; i++)
		{
			...
			std::string input_file  = baseInputFolder.c_str() + 
				Dataset::randomN +"\\"+ std::to_string(actual_dataset_size) +
				"RANDsample"+std::to_string(i).c_str()+"model.txt";
...
Dataset Dataset::constGridDataset()
{
	...
	dataset.output_stats = baseOutputFolder.append("CONSTGRIDheuristicsStats.txt");
