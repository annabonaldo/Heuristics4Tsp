...
std::string GreedySolver::solve(const TSP& tsp, const TSPSolution& initSol, TSPSolution& bestSol)
{
	...
	string solutionNotOptimized = getLine(solSize, problemSize, time);

	if (this->optimized) {
		std::cout << "optimize2opt" << std::endl;
		optimize2opt(tsp, bestSol);
		time = to_string(getWallTime() - startTime);
